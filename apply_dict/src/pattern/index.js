export const getRootPositionListUrl = '/position/get_root_position_list/' // 获取根地点接口
export const getSubPositionListUrl = '/position/get_sub_position_list/' // 获取子地点接口
export const examineApplyUrl = '/apply/examine_apply/' // 审核申请
export const getApplyUrl = '/apply/' // 获取查询集接口
export const getApplyUserUrl = '/organizationmember/get_apply_users/' // 获取组员接口
export const getPositionsUrl = '/position/get_root_position_list/' // 获取一级地区接口
export const getUserInfoUrl = '/user_info/' // 获取用户信息
export const editUserInfoUrl = '/user_info/edit_user_info/' // 修改用户信息
export const delFilesUrl = '/del_excel' // 删除已生成的excel文件接口
export const analysisExcelUrl = '/analysis_apply_excel' // 解析excel文件数据接口
export const commitApplyUrl = '/apply/submit_apply_list/' // 提交申请接口
export const deriveExcelUrl = '/derive_excel' // 导出数据接口
export const getGoodApplyListUrl = '/apply/get_self_good_apply_list/' // 获取申请列表接口
export const editApplyUrl = '/apply/update_good_apply/' // 更新apply信息
export const deleteApplyUrl = 'delete_good_apply/' // 删除apply
export const getApplyStatusUrl = '/apply/get_apply_status/' // 获取所有申请状态
export const getSecretaryUrl = '/secretary/get_org_secretary/' // 获取秘书
export const GetUserOrgIdUrl = '/organizationmember/get_user_org_id/' // 获取user所在的的org_id
